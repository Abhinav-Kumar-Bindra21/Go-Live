...
const ParticipantList = ({ isOpen, onClose, localUser, localAudio, localVideo, remoteUsers, meetingHostId }) => {
  ...
  return (
    <aside className="w-full sm:w-80 h-full bg-white border border-slate-200 flex flex-col z-30 shadow-2xl animate-in slide-in-from-right duration-200">
      {/* Header */}
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-3">
        {allParticipants.map((p) => {
          ...
          return (
            <div
              key={p.socketId}
              className="flex items-center justify-between p3 bg-slate-50 border border-slate-200 shadow-xs"
            >
              <div className="flex items-center gap-3">
                <div className="size-9 rounded-full bg-primary-light border border-primary-border text-primary font-bold flex items-center justify-center text-sm shadow-xs">
              ...
              <div className="flex items-center gap-2 text-slate-500">
                {p.audioEnabled ? (
                  <MicIcon className="size-4 text-slate-600" />
                ) : (
                  <MicOffIcon className="size-4 text-rose-500" />
                )}

                {p.videoEnabled ? (
                  <VideoIcon className="size-4 text-slate-600" />
                ) : (
