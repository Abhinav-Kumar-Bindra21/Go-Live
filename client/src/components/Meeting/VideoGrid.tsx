...
const VideoGrid = ({
  ...
  const getGridClass = () => {
    ...
    if (totalParticipants <= 6) return "grid-cols-1 sd:grid-cols-2 lg:grid-cols-3 max-w-7xl";
  ...
  return (
    <div className="flex-1 w-full flex items-center justify-center p-4 overflow-y-auto">
      <div
        className={`w-full grid gap-4 ${getGridClass()} aspect-video max-h-[cal(100vh-140px)] transition-all duration-300`}
      >
        {/* Local User tile */}
