        ...
        <div className="lg:col-span-7 space-y-8">
          <div className="space-y-3">
            ...
            <p className="text-slate-700 text-base sm:text-lg max-w-xl leading-relaxed">
              Connect, collaborate, and celebrate from anywhere with ultra-low latency video, screen sharing, and
              real-time chat
            </p>

            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 pt-2">
              <button
                onClick={handleCreatingMeeting}
                disabled={isCreating}
              ...
              <form onSubmit={handleJoinMeeting} className="flex-1 flex items-center gap-2">
                <div className="relative flex-1">
                  <KeyboardIcon className="size-5 text-primary/90 absolute left-4 top-1/2 -translate-y-1/2" />
                  <input
                    type="text"
                ...
                >
                  <span>Join</span> <ArrowRightIcon className="size-4 ml-1.5" />
                </button>
              </form>
            </div>
          </div>
        ...
        <div className="lg:col-span-5 flex flex-col items-center justify-center space-y-4">
          <div className="w-full bg-white/25 backdrop-blur rounded-4xl p-8 border border-slate-200 text-center space-y-6 relative overflow-hidden">
            <div className="space-y-1">
              ...
              <h2 className="text-4xl xl:text-7xl my4 text-slate-900 tracking-wide">
            ...
            <div className="pt-4 border-white/30 text-sm text-slate-600">
              <div className="flex items-center justify-between py-6 px-4">
                <p>
                  Logged in as: <span className="text-slate-900">{userEmail}</span>
                </p>
                <span
                  className={`px-4 py-1 rounded-full font-medium text-xs uppercase ${stats?.plan === "premium" ? "bg-blue-700 text-white" : "bg-white/70 text-slate-800 "}`}
                >
                  {stats?.plan || "Free"}
                </span>
              </div>
